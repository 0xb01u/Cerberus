...
exports.run = async (bot, msg, args, serverID) => {
	...
	switch (args[0]) {
		case "join":{
			if (args.length < 2) {
				let reply = await msg.reply("please, let me know what team you want to join! :(");
		...
		case "leave":{
			if (args.length < 2) {
				let reply = await msg.reply("please, let me know what unconfirmed team you want to leave! :(");
